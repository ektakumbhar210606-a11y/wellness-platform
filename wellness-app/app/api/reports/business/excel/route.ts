...
/**
 * POST /api/reports/business/excel - Generate Excel from custom report data
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate Excel
    const excelBuffer = await generateExcel(customReportData, 'business');

    // Return Excel file as downloadable file
    return new NextResponse(excelBuffer, {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="custom_business_report_${Date.now()}.xlsx"`
      },
      status: 200
    });
