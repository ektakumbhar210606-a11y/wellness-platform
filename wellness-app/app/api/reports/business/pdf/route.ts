...
/**
 * POST /api/reports/business/pdf - Generate PDF from custom report data
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const pdfBuffer = await generatePDF(customReportData, 'business', 'Custom Business Report');

    // Return PDF as downloadable file
    return new NextResponse(pdfBuffer, {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="custom_business_report_${Date.now()}.pdf"`
      },
      status: 200
    });
