...
export async function POST(request: NextRequest) {
  try {
    ...
    // const Review = reviewModule.default; // Review is not needed directly here
    ...
    const existingReview = await reviewModule.default.findOne({ booking: booking_id });
    if (existingReview) {
      return NextResponse.json(
    ...
    const newReview = new reviewModule.default({
      booking: booking_id,
      rating: rating,
    ...
    const therapistReviews = await reviewModule.default.find({ 
