...
function BookingSlot() {
  ...
  const fetchAvailableSlots = async () => {
    try {
      ...
      // Fetch available slots from the new available-slots API
      const response = await fetch(`/api/bookings/available-slots?businessId=${businessId}&serviceId=${serviceId}&therapistId=${therapistId}&date=${new Date().toISOString().split('T')[0]}`, {
        method: 'GET',
        headers: {
      ...
      if (result.success) {
        // Process the slots from the new API format
        const today = new Date();
        const todayFormatted = moment(today).format('YYYY-MM-DD');
        
        const processedSlots = result.data.map((slot: any, index: number) => ({
          id: `slot-${index}`,
                  ...
                  availableSlots.map((slot) => {
                    ...
                    return (
                      <Col key={slot.id} xs={24} sm={12} md={8}>
                        <Card
                          hoverable={!isPast && slot.isAvailable}
                          style={{
                            borderRadius: 8,
                            border: selectedSlot?.id === slot.id ? '2px solid #1890ff' : 
                                     isPast || !slot.isAvailable ? '1px solid #ffccc7' : '1px solid #f0f0f0',
                            backgroundColor: selectedSlot?.id === slot.id ? '#e6f7ff' : 
                                           isPast || !slot.isAvailable ? '#f5f5f5' : '#ffffff',
                            cursor: isPast || !slot.isAvailable ? 'not-allowed' : 'pointer',
                            opacity: (isPast || !slot.isAvailable) ? 0.6 : 1
                          }}
                          onClick={() => !isPast && slot.isAvailable && handleSlotSelect(slot)}
                        >
